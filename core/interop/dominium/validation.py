...
def _error(errors: list[str], error_records: list[dict[str, Any]], code: str, path: str, message: str) -> None:
    errors.append(f"{code}: {message}")
    error_records.append({"code": code, "path": path, "message": message})
...
def _check_common_metadata(
    ...
    *,
    ...
    if metadata.get("schema_version") != models.SCHEMA_VERSION:
        _error(errors, error_records, "schema.version", f"{label}.metadata.schema_version", f"{label} schema_version mismatch")
    if not is_commit_sha(metadata.get("source_revision")):
        _error(errors, error_records, "revision.syntax", f"{label}.metadata.source_revision", f"{label} source_revision must be commit sha")
    if expected_revision and metadata.get("source_revision") != expected_revision:
        _error(errors, error_records, "revision.binding", f"{label}.metadata.source_revision", f"{label} source_revision does not match bundle revision")
    ...
    *,
    ...
    for field in sorted(rule.get("required_spec", set())):
        if field not in spec:
            _error(errors, error_records, "spec.required", f"{kind}.spec.{field}", f"{kind} missing spec.{field}")
    ...
    for field in models.FALSE_STATUS_FIELDS:
        if status.get(field) is not False:
            ...
            _error(errors, error_records, code, f"{kind}.status.{field}", f"{kind} status.{field} must be false")
...
def _check_bridge(bundle: dict[str, Any], errors: list[str], error_records: list[dict[str, Any]]) -> None:
    ...
    if command_mapping.get("command_invocation_implemented") is not False:
        _error(errors, error_records, "command.invocation", "records.dominium_bridge_manifest.spec.command_mapping.command_invocation_implemented", "command invocation must not be implemented")
    ...
    if ownership.get("Workbench") != "presentation, context capture, preview, approval interaction, apply requests":
        _error(errors, error_records, "workbench.authority", "records.dominium_bridge_manifest.spec.ownership.Workbench", "Workbench ownership boundary mismatch")
...
def _check_registry_projection(
    ...
    *,
    ...
    if diag_summary.get("native_count") != len(native_diagnostics) or diag_summary.get("projected_count") != len(diagnostic_records):
        _error(errors, error_records, "diagnostic.summary", "registry_projection_summary.diagnostics", "diagnostic projection summary mismatch")
    if ref_summary.get("native_count") != len(native_refusals) or ref_summary.get("projected_count") != len(refusal_records):
        _error(errors, error_records, "refusal.summary", "registry_projection_summary.refusals", "refusal projection summary mismatch")
...
def validate_bundle(bundle: dict[str, Any], *, dominium_root: str | Path | None = None) -> dict[str, Any]:
    ...
    if bundle.get("status", {}).get("generated_projection_marked_canonical") is not False:
        _error(errors, error_records, "authority.canonical_overclaim", "status.generated_projection_marked_canonical", "generated projection marked canonical")
    for field in models.FALSE_STATUS_FIELDS:
        status = bundle.get("status", {}) if isinstance(bundle.get("status"), dict) else {}
        if status.get(field) is not False:
            _error(errors, error_records, "status.false_boundary", f"status.{field}", f"bundle status.{field} must be false")
