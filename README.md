...
- Next AIDE-local work: AIDE-TASK-OS-STATUS-REPAIR-01 repairs stale Task OS current/latest-task reporting; after review, AIDE-APPLY-LIFECYCLE-PLAN-01 may be proposed as planning-only. This does not authorize install, upgrade, repair, rollback/uninstall, target mutation, branch/worktree mutation, release publication, GitHub mutation, provider/model calls, Gateway calls, network calls, or broad active-repo apply.
