...
- Next AIDE-local work: AIDE-LIFECYCLE-SCHEMA-VALIDATOR-01 is a review-gated local validator task after lifecycle schema and fixture-shape planning. It adds report-only `lifecycle-schema` status, validate, and fixture-verify commands plus targeted tests and lifecycle-schema reports. Fixture materialization and lifecycle apply remain deferred; this does not authorize install, upgrade, lifecycle repair, rollback/uninstall, active repo apply, target mutation, branch/worktree mutation, release publication, GitHub mutation, provider/model calls, Gateway calls, network calls, or broad active-repo apply.
