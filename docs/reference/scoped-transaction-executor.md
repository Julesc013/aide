...
Supported modes are `dry-run`, `report`, and explicit `apply`. Dry-run/report mode writes report evidence but does not mutate target files. Apply mode is available only when the transaction plan explicitly requests `mode: apply`; it still fails closed on path, resolved path, operation, marker, preimage hash, postimage verification, staged-change, or rollback-compatible record failures before writing.

Scoped executor v0 does not provide multi-file atomic apply. In apply mode, more than one mutating operation is blocked with `BLOCKED_MULTI_OPERATION_APPLY_NOT_ATOMIC` before any mutation. Dry-run/report mode may still plan multiple operations without target mutation. Persisted reports include their repo-relative `report_path`.
