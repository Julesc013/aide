...
- `Q12`: Verifier v0 for evidence packets, task packets, file refs, line refs, diff scope, adapter drift, context shape, token warnings, and secret-scan heuristics, awaiting review
- `Q13`: Evidence Review Workflow for compact GPT-5.5/human review packets, verifier integration, evidence refs, validation summaries, changed-file summaries, token summaries, risk summaries, and decision policy, awaiting review
- `Q14`: Token Ledger and Savings Report for metadata-only estimated token records, named baselines, budget status, regression warnings, compact summaries, and ledger scan/report/compare commands, awaiting review
- `Q15`: Golden Tasks v0 for deterministic repo-local quality gates over compact task packets, context packets, verifier failure detection, review packets, token ledger metadata, and adapter determinism, awaiting review
- `Q16`: Outcome Controller v0 for deterministic advisory recommendations from token, verifier, review, golden-task, context, and adapter signals, awaiting review
- `Q17`: Router Profile v0 for deterministic advisory route decisions from compact task/context packets, verifier, token, golden-task, and outcome signals, awaiting review
- `Q18`: Cache and Local State Boundary for `.aide.local/` protection, safe example local-state layout, cache/local-state policy, deterministic cache-key metadata, and cache init/status/key/report commands, awaiting review
- `Q19`: Gateway Architecture and Skeleton for local/report-only health, status, route explanation, summaries, and version surfaces without provider/model calls or proxy forwarding, awaiting review
- `Q20`: Provider Adapter v0 for offline provider-adapter contracts, provider family metadata, capability metadata, status artifacts, and provider list/status/validate/contract/probe --offline commands, awaiting review

## Near-Term Phases

- review Q20 Provider Adapter v0 evidence before Q21 Existing Tool Adapter Compiler v0
