...
This task changes queue review state and generated artifacts, so it must stop
at `needs_review` after evidence is written. Review-state changes are made only
where existing task-local evidence supports `passed_with_notes`.
