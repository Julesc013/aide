...
parser.add_argument('--suite', choices=('host', 'importer-recovery'), default='host')
suite = parser.parse_args().suite
importer = suite == 'importer-recovery'
oracle = 'test_export_import.py' if importer else 'test_continuous_worker_host.py'
result_name = 'bounded-importer-recovery-check' if importer else 'bounded-host-check'
with tempfile.TemporaryDirectory(prefix='tiny-aide-validation-', dir=parent) as temporary:
    ...
    config = {'schema': 'aide.managed-workspace.local.v1',
        ...
        'limits': {'disk_reserve_bytes': 10*1024**3, 'physical_reserve_bytes': 4*1024**3,
            ...
            'log_bytes': 65536, 'runtime_seconds': 180 if importer else 120,
    ...
    (evidence/(result_name+'.txt')).write_bytes(log)
