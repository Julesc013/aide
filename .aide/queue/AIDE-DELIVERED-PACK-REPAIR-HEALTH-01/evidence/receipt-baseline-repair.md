...
The superseding source was frozen at `486aa2bd0f5f5255f216bd5f1654278574fe974b`, tree `739d46e6db715c9e11b55e8b6d116ff4a2ab8f95`. It checks every receipt row's canonical kind/source/target and mode against the validated current pack manifest, then checks the receipt source digest against pack bytes. A receipt-owned installed digest must also match the pack baseline, allowing the existing LF/CRLF managed-section representation. Disabled entries cannot claim ownership. An invalid row remains UNKNOWN, has no repair plan, and makes the report PRESERVATION_REQUIRED. An intentional local overlay retains project ownership and unknown rationale, and the top-level report requires preservation rather than claiming whole-install health. The target path is reported lexically without following a rejected root symlink. Independent rereview returned REQUEST_CHANGES for valid CRLF pack template bytes; see `crlf-template-repair.md`.
...
Validation on the superseding source before its freeze in `D:/Projects/AIDE/aide-repair-health`, Windows Python 3:
